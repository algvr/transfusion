        ...
        if len(preds) > 0:
            ...
            if len(labels) > 0:
                ...
                for i in predicted_scores.argsort()[::-1]:
                    ...
                    matchings = self._match({k: p[i] if not (k == "ttcs" and len(p) <= i) else -1.0 for k, p in preds.items()}, labels, overlaps)  # NGT x NR
...
class STAMeanAveragePrecisionNoTTC(AbstractMeanAveragePrecision):
    ...
    def _map_classes(self, preds):
        ...
        nouns = preds['nouns']
        verbs = preds['verbs'] if 'verbs' in preds else ones

        return np.vstack([
            nouns, # map_noun
            verbs, # map_verb
            ones, # ap_box - average over nouns (can't have map_box)
    ...
    def _match(self, pred, gt_predictions, ious):
        ...
        verbs = ((pred['verbs'] == gt_predictions['verbs']) if 'verbs' in pred
                  else np.full_like(gt_predictions['nouns'], False))
        # ttcs = ((np.abs(pred['ttcs'] - gt_predictions['ttcs']) <= self.tti_threshold)
        #          if 'ttcs' in pred else np.full_like(gt_predictions['nouns'], False))
        ...
        return np.vstack([tp_noun,  # map_noun
                          tp_verb,  # map_verb
                          tp_box,  # ap_box
...
class STAMeanAveragePrecision(AbstractMeanAveragePrecision):
    ...
    def __init__(self, iou_threshold=0.5, ttc_threshold=0.25, top_k=5, count_all_classes=False):
        ...
        :param count_all_classes: whether to also average across classes with no annotations. False is the default for many implementations.
        """
        super().__init__(8, top_k=top_k, count_all_classes=count_all_classes)
        self.iou_threshold = iou_threshold
        self.tti_threshold = ttc_threshold
    ...
    def _map_classes(self, preds):
        ...
        return np.vstack([
            nouns, # map_noun
            verbs, # map_verb
            ones, # ap_box - average over boxes (can't have map_box)
            nouns, # map_box_noun - average over nouns
            nouns, # map_box_noun_verb - average over nouns
            nouns, # map_box_noun_ttc - average over nouns
            nouns, # map_box_noun_verb_ttc - average over nouns
            ones, #ap_ttc - average over boxes
        ]).T
